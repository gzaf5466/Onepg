...
router.post('/social', loginLimiter, [
  body('email').isEmail().withMessage('Valid email required.').normalizeEmail(),
  body('provider').isIn(['google', 'github']).withMessage('Unsupported social auth provider.')
], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ success: false, message: errors.array()[0].msg });
...
router.get('/github', (req, res, next) => {
  const clientUrl = (process.env.CLIENT_URL || 'http://localhost:5173').replace(/\/$/, '');
  if (!process.env.GITHUB_CLIENT_ID || process.env.GITHUB_CLIENT_ID === 'DUMMY_GITHUB_CLIENT_ID') {
    console.warn('⚠️ GitHub OAuth attempt failed: GITHUB_CLIENT_ID is not configured in server/.env');
    return res.redirect(`${clientUrl}/login?error=${encodeURIComponent('GitHub Authentication service is currently unavailable. Please sign in with your email and password.')}`);
  }
  passport.authenticate('github', { scope: ['user:email'], session: false })(req, res, next);
});

// GET /api/auth/github/callback — Passport GitHub OAuth Callback
router.get('/github/callback', (req, res, next) => {
  passport.authenticate('github', { session: false }, (err, user) => {
    const clientUrl = (process.env.CLIENT_URL || 'http://localhost:5173').replace(/\/$/, '');
    if (err || !user) {
      console.error('GitHub Passport callback error:', err);
      const msg = err?.message || 'Unable to authenticate with GitHub. Please try again or sign in with your email.';
      return res.redirect(`${clientUrl}/login?error=${encodeURIComponent(msg)}`);
    }
    ...
    res.redirect(`${clientUrl}/login?token=${token}&social=github`);
  })(req, res, next);
});
