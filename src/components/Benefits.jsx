...
export default function Benefits() {
  return (
    <section id="benefits" className="pb-12 sm:pb-16 md:pb-20 relative z-10 bg-[#050505] overflow-hidden border-b border-white/5">
      {/* Background ambient blur */}
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-[#00E5FF]/5 blur-[150px] rounded-full pointer-events-none" />

      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-4 flex flex-col lg:flex-row items-center gap-6 lg:gap-16">
        
        {/* 3D Routing Nodes Asset (Top on Mobile, Right on Desktop) */}
        <motion.div 
          initial={{ opacity: 0, x: 40 }}
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.8 }}
          className="flex-1 w-full relative order-first lg:order-last"
        >
          <div className="relative w-full h-[210px] sm:h-[300px] lg:h-[420px] rounded-2xl sm:rounded-[2rem] border border-white/[0.08] shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden bg-[#050505] flex items-center justify-center group">
            <img 
              src={routingNodes} 
        ...
        <motion.div 
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.8 }}
          className="flex-1 lg:pr-6 text-center lg:text-left order-last lg:order-first"
        >
